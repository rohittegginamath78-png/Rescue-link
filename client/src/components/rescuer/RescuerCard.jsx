    ...
    <div className="card">
      <div className="mb-3 flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2">
          {rescuer.verified && <Badge tone="amber">Verified</Badge>}
      ...
      <div className="mb-4 space-y-2">
        {rescuer.address && (
          <p className="text-xs text-gray-600">{rescuer.address}</p>
        )}
        {rescuer.phone && (
          <p className="text-xs text-gray-600">Phone: {rescuer.phone}</p>
        )}
        {!rescuer.phone && rescuer.whatsapp && (
          <p className="text-xs text-gray-600">
            WhatsApp only: {rescuer.whatsapp}
          </p>
        )}
        {rescuer.matchedCity &&
          rescuer.requestedCity &&
          rescuer.matchedCity !== rescuer.requestedCity && (
            <p className="text-xs text-amber-700">
              Showing nearest verified coverage from {rescuer.matchedCity}.
            </p>
          )}
      </div>
